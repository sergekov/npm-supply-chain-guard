#!/usr/bin/env bash
# Automatically installs git hooks after pnpm install (via "prepare" lifecycle script).
...
#!/usr/bin/env bash
# Auto-installed by prepare script. See: scripts/install-git-hooks.sh
...
HOOK
  ...
  printf '\n# Supply chain security guard (auto-appended by scripts/install-git-hooks.sh)\n%s\n' "$GUARD_LINE" >> "$HOOKS_DIR/pre-commit"
  chmod +x "$HOOKS_DIR/pre-commit"
